...
\includegraphics[width=\linewidth,height=1in]{eps/21DIL_E=f(AGE)-boxplot(Cam=B).eps}
\includegraphics[width=\linewidth,height=1in]{eps/21CONTRAST_E=f(AGE)-boxplot(Cam=B).eps}
%\includegraphics[width=\linewidth,height=1in]{eps/21AREA_E=f(AGE)-boxplot(Cam=B).eps}
\includegraphics[width=\linewidth,height=1in]{eps/21BITS_E=f(AGE)-boxplot(Cam=B).eps}

\vspace{5pt}\hrule\vspace{6pt}


\includegraphics[width=\linewidth,height=1in]{eps/DIL-PA.eps} 
\includegraphics[width=\linewidth,height=1in]{eps/CONTRAST-PA.eps}
\includegraphics[width=\linewidth,height=1in]{eps/BITS-COMPARED.eps} 
...
\figfooter{a} { Boxplots showing  the variation of Dilation, Contrast, and Number of Bits Encoded / Compared for each age
at Enrollment (left) and  Passage (right); }
\figfooter{b}{Boxplots showing  the variation of HDNORM, HDRAW.  Box width is proportional to the population size, data for travellers enrolled with new  ``B''  cameras are used only.}
\label{fHDpassage}}
\end{figure}
...
%This is further seen in the results below.

%Note that the scales in vertical axes are different.  
%The number of subjects per airport and per camera is provided at the top the figure.

%The performance changes due age are similar at each airport and for each camera. 
%The same pattern of performance variation by age  is observed for each airport, 
...
watch-list screening border application in United Emirates have  solidified this belief, %coining the expression to describe the likelihood of false recognition in iris biometrics as the likelihood of ``tossing a coin heads up 249 times  in a row''
...
\cmt{
%Second, in relationship to the aging debate \cite{IET0} and the role of the OPS-XING dataset, 
%which was extracted from the historical log of NEXUS kiosks
%which was made of the historical log from NEXUS kiosks, it is observed that there are other factors that affected the kiosk performance  over time.
%This includes
%in this order of importance: kiosk location, age of a person, and time of day.
 %kiosk location, time of day and seasonal changes, already reported by the UND researchers \cite{Bowyer-BTAS2016}. 
Additionally, 
%the results presented in this paper from 
using the previously unused portions of the OPS-XING dataset,
% that contains travellers' age and image quality information, 
it is shown that
kiosk performance 
